...
本地运行时，刷新页面后会继续读取这个文件，所以规则会保留。  
如果部署在 Streamlit Community Cloud，运行中的实例刷新通常会保留；但平台重启、休眠恢复或重新部署后，云端文件可能回到 GitHub 仓库版本。需要长期保留时，请下载当前佣金率表 CSV，并把它更新到 GitHub 仓库的 `data/commission_rules.csv`。
